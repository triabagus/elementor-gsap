(function () {
	...
	function ensurePlugin() {
		...
		if (typeof window.DrawSVGPlugin === 'undefined') return false;
	...
	function initOne(container) {
		...
		if (!ensurePlugin()) {
			console.warn('GSAP / DrawSVGPlugin belum dimuat untuk Button Draw Underline.');
			return;
		}

		var box = container.querySelector('[data-draw-line-box]');
		if (!box) return;

		var variant = container.dataset.drawVariant || 'random';
		var duration = parseFloat(container.dataset.drawDuration);
		...
		function pickIndex() {
			if (variant !== 'random') {
				...
				if (!isNaN(n) && n >= 1 && n <= svgVariants.length) {
					return n - 1;
				}
			}
			if (nextIndex === null) {
	...
	if (window.elementorFrontend && window.elementorFrontend.hooks) {
		window.elementorFrontend.hooks.addAction(
			'frontend/element_ready/button_draw_underline.default',
